...
const allowedOrigins = [
    ...
    'https://ghana-trust-bank.onrender.com'  // Live Render frontend
];
...
app.use(cors({
    origin: function(origin, callback) {
        ...
        // Allow file:// protocol (local HTML files)
...
app.post('/api/send-custom-email', async (req, res) => {
    console.log('Received send-custom-email request from:', req.headers.origin || 'unknown');
    console.log('Request body:', { to: req.body?.to, subject: req.body?.subject });
    ...
    try {
        ...
        if (!to || !subject || !body) {
            console.log('Missing required fields:', { to: !!to, subject: !!subject, body: !!body });
            return res.status(400).json({ success: false, message: 'To, subject, and body are required' });
        }

        // Use provided HTML or generate from body
        ...
        const info = await sendBrevoEmail(to, subject, htmlContent, body);
...
// Add /send-email endpoint for compatibility with frontend
